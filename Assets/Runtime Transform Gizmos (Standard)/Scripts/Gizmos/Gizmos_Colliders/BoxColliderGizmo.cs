...
namespace RTGStandard
{
    ...
    //-----------------------------------------------------------------------------
    // Name : EGizmoBoxResizeMode (Public Enum)
    // Desc : Defines different modes for resizing a gizmo box.
    //-----------------------------------------------------------------------------
    public enum EGizmoBoxResizeMode
    {
        ByHandle = 0,   // Resize type is determined by the drag handle used
        Uniform         // Resize is always uniform
    }
    #endregion

    #region Public Classes
    //-----------------------------------------------------------------------------
    // Name: BoxColliderGizmo (Public Class)
    // Desc: Implements a box collider gizmo.
    //-----------------------------------------------------------------------------
    public class BoxColliderGizmo : Gizmo
    {
        ...
        GizmoCap[]              mAxisResizeCapsP    = new GizmoCap[3] { new GizmoCap(), new GizmoCap(), new GizmoCap() };
        GizmoCap[]              mAxisResizeCapsN    = new GizmoCap[3] { new GizmoCap(), new GizmoCap(), new GizmoCap() };
        ...
        BoxCollider             mTarget;                                        // The target box collider
        #endregion

        #region Public Properties
        //-----------------------------------------------------------------------------
        // Name: style (Public Property)
        // Desc: Returns or sets the box collider gizmo style.
        //-----------------------------------------------------------------------------
        public BoxColliderGizmoStyle    style { get; set; } = null;

        //-----------------------------------------------------------------------------
        // Name: activeStyle (Public Property)
        // Desc: Returns the active style used by the gizmo. This is 'style' if it was set
        //       to a valid style. Otherwise, it is the active skin's box collider gizmo style.
        //-----------------------------------------------------------------------------
        public BoxColliderGizmoStyle    activeStyle { get { return style == null ? RTGizmos.get.skin.boxColliderGizmoStyle : style; } }

        //-----------------------------------------------------------------------------
        // Name: target (Public Property)
        // Desc: Returns or sets the target box collider.
        //-----------------------------------------------------------------------------
        public BoxCollider  target { get { return mTarget; } set { mTarget = value; } }      
        ...
        public EGizmoBoxResizeMode  resizeMode
        {
            ...
            set 
            {
                ...
                // the collider has the same size along all 3 axes.
                mResizeMode = value; 
                if (mResizeMode == EGizmoBoxResizeMode.Uniform && dragging && IsAxisResizeHandle(dragData.handle))
                    mTarget.size = mTarget.size.SyncComponents(mDrag.desc.axisIndex0);
            }
        }
        #endregion
        ...
        protected override void OnCreateHandles(List<GizmoHandle> handles)
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                // Enable sphere culling
                mAxisResizeCapsP[i].cullSphereEnabled   = true;
                mAxisResizeCapsN[i].cullSphereEnabled   = true;
                mAxisResizeCapsP[i].cullSphereMode      = EGizmoCullSphereMode.Behind;
                mAxisResizeCapsN[i].cullSphereMode      = EGizmoCullSphereMode.Behind;
                mAxisResizeCapsP[i].cullSphereTargets   = EGizmoHandleCullSphereTargets.Render;
                mAxisResizeCapsN[i].cullSphereTargets   = EGizmoHandleCullSphereTargets.Render;
                mAxisResizeCapsP[i].cullHoveredPixels   = false;
                mAxisResizeCapsN[i].cullHoveredPixels   = false;
        ...
        protected override void OnUpdateHandles(EGizmoHandleUpdateReason updateReason, Camera camera)
        {
            ...
            Vector3 objectScale         = mTarget.transform.lossyScale;
            mBox.capStyle.color         = globalStyle.colliderColor;
            mBox.transform.rotation     = mTarget.transform.rotation;
            mBox.transform.position     = mTarget.GetWorldCenter();
            transform.position          = mBox.transform.position;
            transform.rotation          = mBox.transform.rotation;
            mBox.capStyle.boxSize       = mTarget.GetWorldSize();
        ...
        protected override GizmoDrag OnStartDrag(Camera camera)
        {
            ...
            mDragStartSize = mTarget.GetWorldSize();
        ...
        protected override void OnDrag(GizmoDrag drag)
        {
            ...
            if (dragData.handle == mUniformCap) mTarget.SetWorldSize(mDragStartSize + mDrag.totalDrag);
            else
            {
                ...
                // Set collider size
                mTarget.SetWorldSize(worldSize);
            }

            // If snapping is enabled, fix float errors
            if (mDrag.IsSnapEnabled())
                mTarget.size = mTarget.size.FixFloatError();

            // Take pivot into account
            if (dragData.handle != mUniformCap)
            {
                int axisIndex       = mDrag.desc.axisIndex0;
                Vector3 boxSize     = mTarget.GetWorldSize();                                           // World box size
                ...
                mTarget.SetWorldCenter(boxCenter);                                                      // Set world center
            }
        }
        ...
        protected override void OnGUI(Camera camera)
        {
            ...
            {
                // Draw size label
                string text = $"World size: {mTarget.GetWorldSize().ToString("F3")}";
