...
namespace GameMaker.Core.Editor
{
    public abstract class FilterDataManagerHolder<M> : BaseDataManagerHolder<M> where M : IDefinition
    {
        ...
        {
            var itemSources = GetItemSource();
            MakeItemSource(itemSources);
    ...
    public class DropdownFilterHolder : BaseFilterHolder
    {
        private DropdownField _dropdownField;
        private DropdownFilter _dropdownFilter;
        private List<DropdownData> _dropdownDatas;
        public DropdownFilterHolder(VisualElement root, BaseFilter baseFilter,List<DropdownData> dropdownDatas) : base(root, baseFilter)
        {
            _dropdownDatas = dropdownDatas;
            _dropdownField = root.Q<DropdownField>("DropdownField");
            _dropdownField.choices.Clear();

            foreach (var d in _dropdownDatas)
            {
                _dropdownField.choices.Add(d.Value);
            }
        ...
        {
            base.Bind(elementProperty);
            _dropdownFilter = baseFilter as DropdownFilter;
            _dropdownFilter.Value = _dropdownDatas.FirstOrDefault(x => x.Value == _dropdownField.value).Id;
            _dropdownField.RegisterValueChangedCallback((v) =>
            {
                _dropdownFilter.Value = _dropdownDatas.FirstOrDefault(x => x.Value == v.newValue).Id;
                _dropdownFilter.OnValueChanged?.Invoke();
            });
        }
        public class DropdownData
        {
            ...
            public DropdownData(string id, string value)
            {
                _id = id;
                _value = value;
            }
    ...
    public class DropdownFilter : BaseFilter
    {
        ...
        public DropdownFilter(string title, string binding) : base(title, binding)
        {
        }
        public override bool Compare(SerializedProperty serializedProperty)
        {
            ...
            var prop = serializedProperty.FindPropertyRelative(Binding);
            if (prop.propertyType != SerializedPropertyType.String) return true;
            return prop.stringValue == _value;
        }
    }
}
