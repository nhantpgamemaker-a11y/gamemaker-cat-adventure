...
namespace GameMaker.Item.Runtime
{
    ...
    public class ItemDetailDefinition : BaseDefinition
    {
        ...
        public ItemDetailDefinition(string id, string name, string title,string description, Sprite icon, string itemDefinitionId, List<ItemStatDefinitionRef> itemStatDefinitionRefs) :base(id, name, title,description, icon)
        {
        ...
        {
            return new ItemDetailDefinition(GetID(), GetName(), GetTitle(), GetDescription(), GetIcon(),_itemDefinitionId,_itemStatDefinitionRefs.Select(i=> i.Clone() as ItemStatDefinitionRef).ToList());
        }
