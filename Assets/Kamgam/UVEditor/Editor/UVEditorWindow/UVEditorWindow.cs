...
namespace Kamgam.UVEditor
{
    public partial class UVEditorWindow : EditorWindow
    {
        ...
        // Key events are triggered both by the onKeyEvent() and move/drop in UIToolkit (we use both to be extra sure we catch them).
        ...
        private void onKeyEvent(Event evt)
        {
            ...
            {
#if UNITY_EDITOR_OSX
                bool undoControlKeyPressed = evt.command;
#else
                bool undoControlKeyPressed = evt.control;
#endif
                
                // Undo 
                if (evt.type == EventType.KeyDown && evt.keyCode == settings.UndoKey && undoControlKeyPressed)
                {
                    undo();
                    stopKeyEvent = true;
                }
                ...
                // Redo
                if (evt.type == EventType.KeyDown && evt.keyCode == settings.RedoKey && undoControlKeyPressed)
                {
                    redo();
                    stopKeyEvent = true;
                }
                ...
                if (evt.type == EventType.KeyDown && evt.keyCode == KeyCode.Escape)
                {
                    ...
                    // texture edit
                        ...
                        StopTextureEditing();
                    // triangle select
                    else if(_triangleSelectEnabled)
            ...
            if (evt.button == 0)
            {
                ...
                if (!anyHandle() && !_editCropEnabled)
                {
                    ...
                    if (uvs.Count > 0)
                    {
                        ...
                        var tmpMaterials = SelectedGameObject == null ? EmptyMaterials : GetRendererFromGameObject(SelectedGameObject).sharedMaterials;
        ...
        private void onSelectedVerticesChanged()
        {
            ...
            _tmpVertexIndexIsSelected.Clear();
        }
    }
}