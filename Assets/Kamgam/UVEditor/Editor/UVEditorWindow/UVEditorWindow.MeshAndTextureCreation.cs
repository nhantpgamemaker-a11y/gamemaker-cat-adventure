...
namespace Kamgam.UVEditor
{
    public partial class UVEditorWindow : EditorWindow
    {
        private void applyUVChangesToSelectedObjectMesh(bool recordUndo, int uvChannel)
        {
            ...
            // If the selected object is a mesh without a game object then use that mesh as the shared mesh.
            Mesh sharedMesh = GetSharedMeshFromGameObject(SelectedGameObject, _selectedMesh);
            if (sharedMesh == null)
                sharedMesh = _selectedMesh;
        ...
        public static Mesh GetSharedMeshFromGameObject(GameObject go, Mesh fallbackMesh = null)
        {
            if (go == null)
                return fallbackMesh;
