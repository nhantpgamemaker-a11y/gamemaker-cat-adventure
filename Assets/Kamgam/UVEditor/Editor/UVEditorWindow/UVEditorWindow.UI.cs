...
namespace Kamgam.UVEditor
{
    public partial class UVEditorWindow : EditorWindow
    {
        ...
        [System.NonSerialized] public static Material[] EmptyMaterials = Array.Empty<Material>();
        ...
        [FormerlySerializedAs("MouseIsInside")] public bool IsMouseInside;
        ...
        public void CreateGUI()
        {
            ...
            _root.AddToClassList(UtilsEditor.IsLightTheme() ? "light-skin" : "dark-skin");
            ...
            _root.Q<DropdownField>(name: "TrianglesUVFilterDropDown").RegisterValueChangedCallback((e) =>
            {
            ...
            _verticesSelectionButton.style.backgroundColor = ActiveButtonBgColor;
        ...
        private Color ActiveButtonBgColor =>
            UtilsEditor.IsLightTheme() ? new Color(0.6f, 0.78f, 0.78f, 1f) : new Color(0f, 1f, 1f, 0.2f);
        ...
        public void StartTriangleSelecting()
        {
            ...
            // ui
            _triangleSelectionButton.style.backgroundColor =  ActiveButtonBgColor;
        ...
        public void StartTextureEditing()
        {
            ...
            {
                EditorUtility.DisplayDialog("Selection missing!", "Please select at least three UV vertices to let the tool know which texture and sub-mesh you wish to edit.", "OK");
                return;
            }
            ...
            if (_texture != null)
            {
                ...
                if (isTextureEditingPossible())
                {
                    ...
                    _editTextureButton.style.backgroundColor = ActiveButtonBgColor;
        ...
        public void StartCropping()
        {
            if (_selectedVertices.Count == 0)
            {
                EditorUtility.DisplayDialog("Selection missing!", "Please select at least one UV vertex to let the tool know which texture and sub-mesh you wish to edit.", "OK");
                return;
            }
            ...
            if (_texture != null)
            {
                ...
                if (isTextureEditingPossible())
                {
                    ...
                    _editCropTextureButton.style.backgroundColor = ActiveButtonBgColor;
                    foreach (var ve in _cropBottomTools)
                    {
        ...
        private void verticesSelectLinked(bool remove)
        {
            if (_selectedVertices.Count > 0)
            {
                ...
                AddLinked(
                    _selectedVertices,
                    GetSharedMeshFromGameObject(SelectedGameObject, _selectedMesh),
        ...
        private Dictionary<int,bool[]> _tmpVertexIndexIsSelected = new ();
        ...
        private void UpdateWindowContent()
        {
            ...
            if (_selectedMesh == null || (SelectedGameObject != null && _selectedMesh != GetSharedMeshFromGameObject(SelectedGameObject)))
            {
            ...
            if (renderer != null)
            {
                ...
                {
                    var tex = rendererMaterials[_selectedSubMeshIndex].GetMainTexture() as Texture2D;
                    if (tex != null)
                    {
                        textureFound = true;
                        _texture = tex;
            ...
            var offset = _textureMaterial != null ? _textureMaterial.GetMainTextureOffset() : Vector2.zero;
            var scale = _textureMaterial != null ? _textureMaterial.GetMainTextureScale() : Vector2.one;
            ...
            if (hasUVs)
            {
                if (_uvsAreDirty)
                {
                    ...
                    Material[] tmpSharedMaterials = tmpSharedMaterials = renderer == null ? EmptyMaterials : renderer.sharedMaterials;
                    List<Texture> tmpSmMainTextures = new List<Texture>(tmpSharedMaterials.Length);
                    Texture tmpTextureMaterialMainTexture = _textureMaterial != null ? _textureMaterial.GetMainTexture() : null;
                    for (int smIndex = 0; smIndex < tmpSharedMaterials.Length; smIndex++)
                    {
                        if (tmpSharedMaterials[smIndex] != null)
                            tmpSmMainTextures.Add(tmpSharedMaterials[smIndex].GetMainTexture());
                        else
                    ...
                    for (int subMeshIndex = 0; subMeshIndex < _selectedMesh.subMeshCount; subMeshIndex++)
                    {
                        ...
                        for (int v = 0; v < numOfVertices; v += 3)
                        {
                            ...
                            if (tmpSharedMaterials.Length > subMeshIndex)
                            {
                                sameMaterial = tmpSharedMaterials[subMeshIndex] == _textureMaterial;
                                sameTexture = false;
                            ...
                            if (_uvFilter != UVFilter.All && _triangleSelectEnabled && !filterUV(_selectedVertices, vIndex0, vIndex1, vIndex2))
                            {
                                continue;
                            }
                            ...
                            // Count drawn vertices and limit at 65k since that is the maximum UI Toolkit can draw.
                            // Once we have reached the limit we create a new LineDrawerGL and continue drawing in there.
            ...
            if (hasUVs && !_editCropEnabled && (!_editTextureEnabled || (_editTextureEnabled && !showScaleAndRotateGizmos)))
            {
                ...
                _vertexContainer.LineColor = UVLineColor;
                ...
                var rendererWithMaterials = GetRendererFromGameObject(SelectedGameObject);
                Material[] tmpMaterials = EmptyMaterials;
                if (rendererWithMaterials != null)
                {
                    tmpMaterials = rendererWithMaterials.sharedMaterials;
                }
                ...
                for (int subMeshIndex = 0; subMeshIndex < _selectedMesh.subMeshCount; subMeshIndex++)
                {
                    ...
                    // Update selected vertices cache if needed
                    if (!_tmpVertexIndexIsSelected.ContainsKey(subMeshIndex) ||
                        _tmpVertexIndexIsSelected[subMeshIndex].Length != numOfVertices)
                    {
                        var states = new bool[numOfVertices];
                        for (int v = 0; v < numOfVertices; v++)
                        {
                            // Skip vertices of other sub meshes if edit-texture mode is enabled.
                            if (_editTextureEnabled && tmpMaterials.Length > subMeshIndex &&
                                tmpMaterials[subMeshIndex] != _textureMaterial)
                            {
                                states[v] = false;    
                                continue;
                            }
                            ...
                            states[v] = _selectedVertices.Contains(index);
                        }

                        _tmpVertexIndexIsSelected[subMeshIndex] = states;
                    }
                    ...
                    {
                        // Skip vertices of other sub meshes if edit-texture mode is enabled.
                        if (_editTextureEnabled && tmpMaterials.Length > subMeshIndex &&  tmpMaterials[subMeshIndex] != _textureMaterial)
                            continue;
                        ...
                        bool isSelected = _tmpVertexIndexIsSelected[subMeshIndex][v]; //_selectedVertices.Contains(index);

                        // Draw selected vertices (limit to 65k, 6 vertices [quad] in UI per UV vertex)
        ...
        /// <returns></returns>
        private bool filterUV(List<int> selectedVertices, int vIndex0, int vIndex1, int vIndex2)
        {
            if (_uvFilter == UVFilter.Selected)
            {
                int foundCounter = 0;
                int count = selectedVertices.Count;
                for (int i = 0; i < count; i++)
                {
                    var index = selectedVertices[i];
                    if (index == vIndex0 || index == vIndex1 || index == vIndex2)
                    {
                        foundCounter++;
                        if (foundCounter >= 3)
                        {
