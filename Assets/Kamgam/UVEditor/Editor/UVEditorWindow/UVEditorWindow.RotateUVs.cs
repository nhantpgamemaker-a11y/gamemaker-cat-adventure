...
namespace Kamgam.UVEditor
{
    public partial class UVEditorWindow : EditorWindow
    {
        ...
        [System.NonSerialized] protected float _totalRotationSinceMousePress = 0f;
        [System.NonSerialized] protected float _totalRotationSinceMousePressTarget = 0f;
        ...
        private void startRotatingUVs(int cornerHandle, MouseDownEvent evt)
        {
            ...
            _totalRotationSinceMousePress = 0f;
        ...
        private void rotateUVs(int rotationHandle, MouseMoveEvent evt)
        {
            ...
            var angleInDeg = Vector2.SignedAngle(centerToMouseOld, centerToMouseNew);
            
            // If shift is pressed then snap to 15 degrees
            _totalRotationSinceMousePressTarget += angleInDeg;
            if (evt.shiftKey)
            {
                var snapAngle = Mathf.RoundToInt(_totalRotationSinceMousePressTarget / 15f) * 15f;
                var deltaToSnapAngle = snapAngle - _totalRotationSinceMousePress;
                if (Mathf.Abs(deltaToSnapAngle) >= 15f)
                {
                    _totalRotationSinceMousePressTarget -= angleInDeg;
                    angleInDeg = deltaToSnapAngle;
                    _totalRotationSinceMousePressTarget += angleInDeg;
                    _totalRotationSinceMousePress = _totalRotationSinceMousePressTarget;
                }
                else
                {
                    angleInDeg = 0F;
