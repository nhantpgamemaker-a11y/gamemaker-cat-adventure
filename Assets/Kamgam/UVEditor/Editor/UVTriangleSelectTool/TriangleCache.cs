...
namespace Kamgam.UVEditor
{
    ...
    /// Occlusion culling is a problem for objects with many triangles. It's not supported.
    /// <br /><br />
    /// NOTICE: Some of the calculations are done in WORLD SPACE. This means the mesh vertext coordinates are
    /// transformed from and to world space. This can lead to selection inaccuracies if the object is positioned
    /// far from the origin (0/0/0). Distances as small as 1000 units can already lead to noticable position deltas
    /// (the selection raycast will be off).
    /// </summary>
    public static class TriangleCache
    {
        ...
        static Dictionary<Component, List<Vector2>> _cachedUV0s = new Dictionary<Component, List<Vector2>>();
        static Dictionary<Component, List<Vector2>> _cachedUV1s = new Dictionary<Component, List<Vector2>>();
        static Dictionary<Component, List<Vector2>> _cachedUV2s = new Dictionary<Component, List<Vector2>>();
        static Dictionary<Component, List<Vector2>> _cachedUV3s = new Dictionary<Component, List<Vector2>>();
        static Dictionary<Component, List<Vector2>> _cachedUV4s = new Dictionary<Component, List<Vector2>>();
        ...
        private static void bakeMeshIfNeeded(SkinnedMeshRenderer meshRenderer)
        {
            if (!_bakedMeshesCache.ContainsKey(meshRenderer) || _bakedMeshesCache[meshRenderer] == null)
            {
                ...
                if(_bakedMeshesCache.ContainsKey(meshRenderer))
                {
                    _bakedMeshesCache.Remove(meshRenderer);
                }
        ...
        public static Mesh GetCachedMesh(Component component)
        {
            if (component == null)
                return null;
            ...
            {
                return _cachedMeshes[component];
            }
        ...
        /// <summary>
        /// Returns null if no cache exists for this component or sub mesh index.
        /// </summary>
        /// <param name="component"></param>
        /// <param name="subMeshIndex"></param>
        /// <returns></returns>
        public static List<int> GetCachedTriangles(Component component, int subMeshIndex)
        {
            if (component == null)
                return null;

            if (_cachedTrianglesCache.ContainsKey(component))
            {
                var subMeshes = _cachedTrianglesCache[component];
        ...
        [MenuItem("Tools/UV Editor/Debug/Cache Triangles", priority = 301)]
        ...
        private static void getMeshObjects(IList<GameObject> objects, out SkinnedMeshRenderer[] skinnedMeshRenderers, out MeshFilter[] meshFilters)
        {
            ...
            if (objects == null)
            {
                ...
                else
                {
                    ...
                    skinnedMeshRenderers = Transform.FindObjectsByType<SkinnedMeshRenderer>(FindObjectsInactive.Exclude, FindObjectsSortMode.InstanceID);
                    meshFilters = Transform.FindObjectsByType<MeshFilter>(FindObjectsInactive.Exclude, FindObjectsSortMode.InstanceID);
#else
        ...
        private static void cacheMesh(Camera cam, Component component, Mesh mesh)
        {
            if (_cachedMeshes.ContainsKey(component))
            {
                ...
                _cachedUV0s.Remove(component);
                _cachedUV1s.Remove(component);
                _cachedUV2s.Remove(component);
                _cachedUV3s.Remove(component);
                _cachedUV4s.Remove(component);
            }
        ...
        public static void BuildUVCache(Component component, Mesh mesh)
        {
            if (component == null)
                return;
            ...
            mesh.GetUVs(0, uv0s);
            if (_cachedUV0s.ContainsKey(component))
                _cachedUV0s.Remove(component);
            _cachedUV0s.Add(component, uv0s);

            var uv1s = new List<Vector2>();
            mesh.GetUVs(1, uv1s);
            if (_cachedUV1s.ContainsKey(component))
                _cachedUV1s.Remove(component);
            _cachedUV1s.Add(component, uv1s);

            var uv2s = new List<Vector2>();
            mesh.GetUVs(2, uv2s);
            if (_cachedUV2s.ContainsKey(component))
                _cachedUV2s.Remove(component);
            _cachedUV2s.Add(component, uv2s);

            var uv3s = new List<Vector2>();
            mesh.GetUVs(3, uv3s);
            if (_cachedUV3s.ContainsKey(component))
                _cachedUV3s.Remove(component);
            _cachedUV3s.Add(component, uv3s);

            var uv4s = new List<Vector2>();
            mesh.GetUVs(4, uv4s);
            if (_cachedUV4s.ContainsKey(component))
                _cachedUV4s.Remove(component);
            _cachedUV4s.Add(component, uv4s);
        }
        ...
        public static Mesh GetBakedMeshForComponent(Component component)
        {
            if (_bakedMeshesCache.ContainsKey(component))
            {
                return _bakedMeshesCache[component];
            }

            return null;
        }

        public static void GetRenderers(IList<GameObject> objects, IList<MeshRenderer> renderers, IList<SkinnedMeshRenderer> skinnedRenderers)
        {
            if (renderers == null)
                return;
            ...
            IList<RayCastTriangleResult> results)
        {
            ...
            // If mesh is invalid then remove and update cache.
            if (mesh == null)
            {
                _cachedMeshes.Remove(comp);
                CacheTriangles();
            }
            
            // If mesh is still not cached then abort
            if (!_cachedMeshes.ContainsKey(comp))
                return;
            mesh = _cachedMeshes[comp];
            if (mesh == null)
                return;
        ...
        /// <param name="uvLayoutIndex">The UV channel index to use for comparison.</param>
        /// <returns></returns>
        public static List<SelectedTriangle> AddLinked(
            IList<GameObject> objects, SelectedTriangle selectedTri,
            bool limitToSubMesh,
            int maxDuration,
            ...
            )
        {
            ...
            try
            {
                foreach (var comp in components)
                {
                    ...
                    List<Vector2> uvs = GetCachedUVs(comp, uvLayoutIndex);
                        ...
                        while (!completed)
                        {
                            ...
                            for (int subMeshIndex = 0; subMeshIndex < trisPerSubMesh.Count; subMeshIndex++)
                            {
                                ...
                                for (int i = 0; i < triCount; i += 3)
                                {
                                    ...
                                    {
                                        // Do any of the vertices overlap?
                                        ...
                                        bool uvCompare = false;
                                        if (compareConnectedUVs)
                                        {
                                            uvCompare = V3Equal(uvs[idx0], uvs[lastConnectedVertices[v]])
                                                     || V3Equal(uvs[idx1], uvs[lastConnectedVertices[v]])
                                                     || V3Equal(uvs[idx2], uvs[lastConnectedVertices[v]]);
                                        }
                                        else
                                        {
                                            vertexCompare = V3Equal(vertices[idx0], vertices[lastConnectedVertices[v]])
                                                         || V3Equal(vertices[idx1], vertices[lastConnectedVertices[v]])
                                                         || V3Equal(vertices[idx2], vertices[lastConnectedVertices[v]]);
                                        }

                                        if (vertexCompare || uvCompare)
                                        {
                        ...
                        if (watch.ElapsedMilliseconds > maxDurationInMilliSec)
                        {
                            EditorApplication.delayCall += () =>
                            {
                                string msg = "Sorry, the mesh is too complex for linked polygon search (too many triangles)." +
                                    ...
                                    "\n\nYou can try to combine multiple selections starting from different spots." +
        ...
        public static List<Vector2> GetCachedUVs(Component comp, int uvLayoutIndex)
        {
            switch (uvLayoutIndex)
            {
                case 0:
                    return getCachedUVs(_cachedUV0s, comp);
                case 1:
                    return getCachedUVs(_cachedUV1s, comp);
                case 2:
                    return getCachedUVs(_cachedUV2s, comp);
                case 3:
                    return getCachedUVs(_cachedUV3s, comp);
                case 4:
                    return getCachedUVs(_cachedUV4s, comp);
                default:
                    return getCachedUVs(_cachedUV0s, comp);
            }

            throw new System.Exception($"TriangleCache: UV index {uvLayoutIndex} not supported.");
        }

        public static Vector2 GetCachedUV(Component comp, int vertexIndex, int uvLayoutIndex)
        {
            switch (uvLayoutIndex)
            {
                case 0:
                    return getCachedUV(_cachedUV0s, comp, vertexIndex);
                case 1:
                    return getCachedUV(_cachedUV1s, comp, vertexIndex);
                case 2:
                    return getCachedUV(_cachedUV2s, comp, vertexIndex);
                case 3:
                    return getCachedUV(_cachedUV3s, comp, vertexIndex);
                case 4:
                    return getCachedUV(_cachedUV4s, comp, vertexIndex);
                default:
                    return getCachedUV(_cachedUV0s, comp, vertexIndex);
            }
        ...
        static Vector2 getCachedUV(Dictionary<Component, List<Vector2>> uvs, Component comp, int vertexIndex)
        {
            if (uvs.ContainsKey(comp) && uvs[comp].Count > vertexIndex)
            {
                return uvs[comp][vertexIndex];
            }

            return Vector2.zero;
        }

        static List<Vector2> getCachedUVs(Dictionary<Component, List<Vector2>> uvs, Component comp)
        {
            if (uvs.ContainsKey(comp))
            {
                return uvs[comp];
            }
        ...
        public static void SetCachedUVs(Component comp, int uvIndex, List<Vector2> uvWorkingCopy)
        {
            var uvs = GetCachedUVs(comp, uvIndex);

            // No uvs to update found
            if (uvs == null)
                return;

            uvs.Clear();
            int numOfUVs = uvWorkingCopy.Count;
            for (int i = 0; i < numOfUVs; i++)
            {
                uvs.Add(uvWorkingCopy[i]);
            }
        }

        public static RayCastTriangleResult CreateResultFromVertices(Component comp, int vertex0, int vertex1, int vertex2)
        {
            if (_cachedTrianglesCache.ContainsKey(comp))
            {
                var componentKV = _cachedTrianglesCache[comp];
                foreach (var subMeshKV in componentKV)
                {
                    int numOfVertices = subMeshKV.Value.Count;
                    for (int i = 0; i < numOfVertices; i += 3)
                    {
                        int v0 = subMeshKV.Value[i];
                        int v1 = subMeshKV.Value[i + 1];
                        int v2 = subMeshKV.Value[i + 2];

                        if (v0 == vertex0 && v1 == vertex1 && v2 == vertex2)
                        {
                            bool isFrontFacing = _cachedTrisFrontFacing[comp][subMeshKV.Key][i / 3];
                            var tri = new RayCastTriangleResult(
                                success: true,
                                comp.transform,
                                GetCachedMesh(comp),
                                comp,
                                subMeshKV.Key,
                                new Vector3Int(v0, v1, v2),
                                comp.transform.InverseTransformPoint(_cachedVerticesInWorldSpaceCache[comp][v0]),
                                comp.transform.InverseTransformPoint(_cachedVerticesInWorldSpaceCache[comp][v1]),
                                comp.transform.InverseTransformPoint(_cachedVerticesInWorldSpaceCache[comp][v2]),
                                !isFrontFacing,
                                _cachedVerticesInViewport[comp][v0].z
                            );
                            return tri;
                        }
                    }
            ...
            RayCastTriangleResult failedTri = default;
            failedTri.Success = false;
            return failedTri;
        }

        public static bool IsFrontFacing(Component component, int vIndex0, int vIndex1, int vIndex2)
        {
            if (_cachedTrisFrontFacing.ContainsKey(component) && _cachedTrianglesCache.ContainsKey(component))
            {
                var subMeshTris = _cachedTrianglesCache[component];
                var subMeshFronts = _cachedTrisFrontFacing[component];
                foreach (var subMesh in subMeshTris)
                {
                    var tris = subMesh.Value; // 3 entries (vertex indices) per tri
                    var isFrontInfos = subMeshFronts[subMesh.Key]; // 1 entry per tri
                    int numOfVertices = tris.Count;
                    int triIndex = 0;
                    for (int i = 0; i < numOfVertices; i += 3)
                    {
                        int v0 = subMesh.Value[i];
                        int v1 = subMesh.Value[i + 1];
                        int v2 = subMesh.Value[i + 2];

                        if (v0 == vIndex0 && v1 == vIndex1 && v2 == vIndex2)
                        {
                            return isFrontInfos[triIndex];
                        }
        ...
        // Just a helper method which is not used by the cache itself but it fits in here.
