...
namespace Kamgam.UVEditor
{
    ...
    partial class UVTriangleSelectTool
    {
        ...
        string getSkinnedIcon(string icon)
        {
        ...
        void drawWindowContent(int controlID)
        {
            ...
            // Fix white fonts as default in Unity light mode.
            if (UtilsEditor.IsLightTheme() && GUI.contentColor.r > 0.5f)
            ...
            var settingsBtnContent = EditorGUIUtility.IconContent(getSkinnedIcon("d_Settings"));
            ...
            closeBtnStyle.normal.background = BackgroundTexture.Get(UtilsEditor.IsLightTheme() ? new Color(0.6f, 0.6f, 0.6f) : bgColor);
            closeBtnStyle.hover.background = BackgroundTexture.Get(new Color(0.7f, 0.7f, 0.7f));
#if UNITY_2023_1_OR_NEWER
            var closeBtnContent = EditorGUIUtility.IconContent(getSkinnedIcon("d_clear@2x"));
#else
            var closeBtnContent = EditorGUIUtility.IconContent(getSkinnedIcon("d_winbtn_win_close_a@2x"));
#endif
        ...
        void drawSelectWindowContentGUI(UVEditorSettings settings)
        {
            ...
            if (DrawButton("", "Show SELECT options.", getSkinnedIcon("d_pick@2x"), "ButtonLeft", GUILayout.Width(24), GUILayout.Height(24)))
            {
                _saveLoadVisibility = 0;
                GUI.FocusControl("BG");
            }
            GUI.enabled = _saveLoadVisibility != 1;
            if (DrawButton("", "Show SAVE options.", getSkinnedIcon("d_CloudConnect@2x"), "ButtonMid", GUILayout.Width(24), GUILayout.Height(24)))
            {
                _saveLoadVisibility = 1;
                GUI.FocusControl("BG");
            }
            GUI.enabled = _saveLoadVisibility != 2;
            if (DrawButton("", "Show LOAD options.", getSkinnedIcon("d_scrolldown@2x"), "ButtonRight", GUILayout.Width(24), GUILayout.Height(24)))
            {
                _saveLoadVisibility = 2;
                GUI.FocusControl("BG");
            }
            GUI.enabled = true;
            if (DrawButton("", "Quicksave: Saves the current selection into the last saved selection without asking for confirmation.", getSkinnedIcon("d_FrameCapture@2x"), null, GUILayout.Width(24), GUILayout.Height(24)))
            {
